...
#>
   ...
   param(
      ...
      # The SharePoint Online region to connect to. Default is Default. Supported values include O365China, O365Default, O365GermanyCloud, O365USGovDoD, O365USGovGCCHigh.
      [ValidateSet('Default', 'ITAR', 'Germany', 'China')]
      [string]$SPORegion = $null,
   ...
   DynamicParam {
      if($Service -contains 'SharePointOnline' -or $Service -contains 'All') {
            ...
            $ParamAttributesCollection = New-Object -Type System.Collections.ObjectModel.Collection[System.Attribute]
            $ParamAttributesCollection.Add($ParamAttributes)
            $DynParam1 = New-Object -Type System.Management.Automation.RuntimeDefinedParameter("SPOAdminUrl", [string], $ParamAttributesCollection)
            $ParamDictionary = New-Object -Type System.Management.Automation.RuntimeDefinedParameterDictionary
            $ParamDictionary.Add("SPOAdminUrl", $DynParam1)
            return $ParamDictionary
      }
   }

   begin {
      $SPOAdminUrl = $PSBoundParameters['SPOAdminUrl']
   }
      ...
      switch ($OrderedImport.Name) {
         ...
         'Microsoft.Online.SharePoint.PowerShell' {
            if ($Service -contains 'SharePointOnline' -or $Service -contains 'All') {
               ...
               try {
                  if($SPORegion) {
                     Connect-SPOService -Url $SPOAdminUrl -Region $SPORegion
                  } else {
